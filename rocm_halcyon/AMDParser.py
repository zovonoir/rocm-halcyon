        ...
        for item in tqdm.tqdm(all_kernels,desc="scrutrizing all device kernels..."):
            ...
            self.all_kernels.append(
                Kernel(
                    ...
                    device = "UNKNOWN" if "device" not in item["args"] else item["args"]["device"],
                    stream = "UNKNOWN" if "stream" not in item["args"] else item["args"]["stream"],
                    correlation = correlation,
                    grid = "UNKNOWN" if "grid" not in item["args"] else item["args"]["grid"],
                    block = "UNKNOWN" if "block" not in item["args"] else item["args"]["block"],
                    smem = smem,
